...
def main():
    ...
    mean_per_class = g.transform('mean')
    std_per_class = g.transform('std')
    std_per_class = std_per_class.replace(0, 1)
    df['zscore'] = ((df['score'] - mean_per_class) / std_per_class).round(3)
    # 5) 统计与导出
    ...
    summary = df.groupby(['class', 'level']).agg({
        'age': 'mean',
        'score': 'mean'
    }).rename(columns={
        'age': 'avg_age',
